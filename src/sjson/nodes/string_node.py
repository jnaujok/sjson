...
from sjson.dictionary.tag_dictionary import TagDictionary  # type: ignore
...
class StringNode(Node):
    ...
    SPECIAL_HANDLING_RLE: BitArray = BitArray(bin="011", length=SPECIAL_HANDLING_LENGTH)
    ...
    def _string_to_binary(self) -> BitArray:
        ...
        """
        # If it's a UUID, we need to convert it to binary
        if self._is_uuid():
            log.debug(f"Encoding UUID {self.value}")
            uuid_bits = self._uuid_to_binary()
        else:
            # Otherwise set the UUID encoding to the full string.
            uuid_bits = self._encode_string()

        # Let's LZ4 compress the string to see if that's shorter (Note: LZ4 can only
        # effectively compress strings of 27 or more characters.)
        if len(self.value) > 26:
            compressed_bits = self._encode_lz4()
        else:
            compressed_bits = self._encode_string()

        # Now let's try run-length encoding the string
        rle_compressed = self._encode_rle()
        ...
        log.debug(
            ...
            f"uuid[{uuid_bits.length}] "
            f"rle[{rle_compressed.length}] "
            f"lz4[{compressed_bits.length}] "
            f"range[{range_compressed.length}]"
        )

        if (
            uuid_bits.length < data_bits.length
            and uuid_bits.length < rle_compressed.length
            and uuid_bits.length < compressed_bits.length
            and uuid_bits.length < range_compressed.length
        ):
            log.debug(f"UUID compressing: {data_bits.length} to {uuid_bits.length}")
            data_bits = uuid_bits
        elif (
            range_compressed.length < data_bits.length
            and range_compressed.length < rle_compressed.length
            ...
            rle_compressed.length < data_bits.length
            and rle_compressed.length < compressed_bits.length
        ):
            log.debug(f"RLE compressing: {data_bits.length} to {rle_compressed.length}")
            data_bits = rle_compressed
    ...
    def _encode_rle(self) -> BitArray:
        rle_encoded = BitArray()
        last_char = None
        count = 0
        for char in self.value:
            if char == last_char:
                ...
                if last_char is not None:
                    rle_encoded += NybbleField.to_binary(count)
                    rle_encoded += BitArray(last_char.encode("utf-8"))
                last_char = char
                count = 1
        if last_char is not None:
            rle_encoded += NybbleField.to_binary(count)
        ...
        return (
            BitArray(StringNode.NEEDS_SPECIAL_HANDLING)
            + StringNode.SPECIAL_HANDLING_RLE
            + NybbleField.to_binary(len(rle_encoded.bin))
            + rle_encoded
        )
    ...
    def _special_handling_to_value(
        ...
        match special_handling_type:
            ...
            case self.SPECIAL_HANDLING_RLE.bin:
                return self._decode_rle(bits)
    ...
    def _decode_rle(self, bits: BitArray) -> str:
        """
        Decodes a RLE string from the bitstream.

        Args:
            bits (BitArray): The raw bitstream being decoded.

        Returns:
            str: The decoded RLE string.
        """
        length: int = NybbleField.to_value(bits)
        data = bits[:length]
        del bits[:length]
        self.value = ""

        while data.length > 0:
            count: int = NybbleField.to_value(data)
            self.value += data[:8].tobytes().decode("utf-8") * count
            del data[:8]
